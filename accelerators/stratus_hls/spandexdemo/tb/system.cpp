...
void system_t::config_proc()
{
    ...
    load_memory();
    {
        ...
        config.owner = owner;
        config.owner_pred = owner_pred;
        config.stride_size = stride_size;
        config.coh_msg = coh_msg;
        config.array_length = array_length;
        config.req_type = req_type;
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - spandexdemo");
        ...
        ESP_REPORT_TIME(end_time, "END - spandexdemo");
...
#else
    in_words_adj = round_up(element_size, DMA_WORD_PER_BEAT);
    out_words_adj = round_up(element_size, DMA_WORD_PER_BEAT);
#endif
