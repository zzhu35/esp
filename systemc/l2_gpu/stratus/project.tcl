...
#
define_hls_module l2_gpu ../src/l2_gpu.cpp
...
foreach sets [list 32] {
	...
	foreach wbits [list 2] {
		...
		foreach abits [list 32] {
		    ...
		    define_sim_config "BEHAV$pars" "l2_gpu BEH" \
			...
			define_hls_config l2_gpu $cname --clock_period=$CLOCK_PERIOD $COMMON_HLS_FLAGS \
			    -DHLS_DIRECTIVES_$cfg -io_config $iocfg

			if {$TECH_IS_XILINX == 1} {

			    define_sim_config "$cname\_V" "l2_gpu RTL_V $cname" "tb TESTBENCH$pars" \
