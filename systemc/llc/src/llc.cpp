...
inline bool llc::is_amo(coh_msg_t coh_msg)
{
        bool ret = false;
        switch (coh_msg)
        {
...
// lookup reqs buf
void llc::reqs_lookup(addr_breakdown_llc_t br, sc_uint<LLC_REQS_BITS> &reqs_hit_i)
{
    LLC_REQS_LOOKUP;
...
bool llc::reqs_peek_req(addr_breakdown_llc_t br, sc_uint<LLC_REQS_BITS> &reqs_empty_i)
{
    LLC_REQS_PEEK_REQ;
        ...
        //         break;
            ...
            else
            {
                if (!recall_valid && !recall_pending) {
                        ...
                        if (states_buf[way] == LLC_S && dma_req_in.coh_msg == REQ_DMA_WRITE_BURST) {
                                int cnt = send_inv_with_sharer_list(addr_evict, sharers_buf[way]);
                                ...
                                recall_pending = true;
                                dbg_recall_pending.write(recall_pending);
                        } else if ((states_buf[way] == LLC_V) && send_fwd_with_owner_mask(FWD_RVK_O, addr_evict, dma_req_in.req_id, owners_buf[way], 0))
                        {
